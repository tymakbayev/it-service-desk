...
const StyledAlert = styled.div`
  ...
  .alert-title {
    font-weight: 600;
    margin-bottom: ${props => props.description ? '0.25rem' : '0'};
  }
...
const Alert = forwardRef(({
  ...
  const IconComponent = icon === true ? colorMap[variant].icon : icon;
    ...
    >
      {icon && (
        <span className="alert-icon">
          {React.isValidElement(icon) ? icon : <IconComponent />}
        </span>
      )}
      ...
      <div className="alert-content">
        ...
        {description && !children && <div className="alert-description">{description}</div>}
      </div>
...
Alert.propTypes = {
  ...
  /** Alert title displayed above the description */
  title: PropTypes.node,
  /** Alert content (can be used instead of description) */
  children: PropTypes.node,
  /** Alert description */
  description: PropTypes.node,
  /** Whether to show the icon or custom icon element */
  icon: PropTypes.oneOfType([PropTypes.bool, PropTypes.node]),
  /** Whether the alert can be dismissed */
  dismissible: PropTypes.bool,
  /** Whether the alert should auto close after a delay */
